...
from src.tools.local_finance_model import score_financial_news
...
async def news_agent(state: AgentState) -> AgentState:
    ...
    try:
        ...
        try:
            ...
            # The news agent only needs the crawler. Exposing all financial
            # tools makes a ReAct model prone to irrelevant/repeated loops.
            mcp_tools = [tool for tool in mcp_tools if tool.name == "crawl_news"]
            logger.info("NewsAgent MCP tools restricted to crawl_news.")

            logger.info(
                f"{SUCCESS_ICON} NewsAgent: Successfully loaded {len(mcp_tools)} tools.")

            all_tools = [*mcp_tools, score_financial_news]
...
2. 对每条新闻先生成简洁、忠实的摘要，然后必须调用 score_financial_news 工具
3. 情感分和风险分必须逐条采用 score_financial_news 返回的 score，不允许自行估分或修改工具分数
4. 分析新闻对股价的潜在影响
5. 识别关键新闻事件和趋势
6. 提供基于新闻的综合投资建议

请使用可用的工具获取实际新闻数据。最终报告必须为每条新闻列出标题、摘要、来源、日期、情感分和风险分，并注明评分来自本地Qwen3-8B QLoRA模型。如果某些新闻无法获取，请基于可用信息提供尽可能全面的分析。"""

            # A bounded protocol prevents repeated calls and recursion-limit
            # failures while preserving per-item local QLoRA scoring.
            agent_input = f"""
Analyze recent financial news for {company_name} ({stock_code}).
Current time: {current_time_info}

STRICT TOOL PROTOCOL (highest priority):
1. Call crawl_news exactly once with query={stock_code.split('.')[-1]!r} and top_k=3.
2. Select at most three usable items from that one result.
3. For each selected item, summarize its title and content faithfully, then call
   score_financial_news exactly once. Never call it more than three times total.
4. Use the sentiment score, risk score, and probabilities returned by the local
   Qwen3-8B QLoRA adapters without altering them.
5. Never call crawl_news again and never score the same item twice.
6. Immediately produce the final report after scoring; make no more tool calls.

Write the final report in clear Chinese. For every item include title, time,
source, concise summary, sentiment score, risk score, and likely market impact.
End with a cross-item conclusion and state that model scores are experimental,
not investment advice. If no usable news is returned, state that and stop.
"""
