...
# The upstream BaoStock Python client stores its socket/session in module-level
# globals. Parallel calls can overwrite each other's connection state, so every
# login-query-logout lifecycle must be serialized inside the MCP server process.
_BAOSTOCK_SESSION_LOCK = threading.RLock()

# --- Baostock上下文管理器 ---
@contextmanager
def _baostock_login_context_unlocked():
...
def baostock_login_context():
    """Serialize access to BaoStock's process-global network session."""
    with _BAOSTOCK_SESSION_LOCK:
        with _baostock_login_context_unlocked():
            yield
